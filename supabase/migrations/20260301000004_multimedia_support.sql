...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'recipe_step' AND policyname = 'Public can view recipe steps') THEN
    CREATE POLICY "Public can view recipe steps" ON recipe_step FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'recipe_step' AND policyname = 'Authenticated users can manage recipe steps') THEN
    CREATE POLICY "Authenticated users can manage recipe steps" ON recipe_step FOR ALL TO authenticated
    USING (EXISTS (SELECT 1 FROM recipe r WHERE r.id = recipe_step.recipe_id AND r.creator_id = auth.uid()));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'recipe_step_media' AND policyname = 'Public can view step media') THEN
    CREATE POLICY "Public can view step media" ON recipe_step_media FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'recipe_step_media' AND policyname = 'Authenticated users can manage step media') THEN
    CREATE POLICY "Authenticated users can manage step media" ON recipe_step_media FOR ALL TO authenticated
    USING (EXISTS (SELECT 1 FROM recipe_step rs JOIN recipe r ON r.id = rs.recipe_id WHERE rs.id = recipe_step_media.step_id AND r.creator_id = auth.uid()));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ingredient' AND policyname = 'Public can view ingredient images') THEN
    CREATE POLICY "Public can view ingredient images" ON ingredient FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ingredient' AND policyname = 'Authenticated users can manage ingredient images') THEN
    CREATE POLICY "Authenticated users can manage ingredient images" ON ingredient FOR UPDATE TO authenticated USING (true);
  END IF;
END $$;
