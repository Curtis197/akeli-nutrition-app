...
CREATE TABLE IF NOT EXISTS app_translation_key (
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'app_translation_key' AND policyname = 'public reads app_translation_key') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'app_translation' AND policyname = 'public reads app_translation') THEN
    CREATE POLICY "public reads app_translation" ON app_translation FOR SELECT USING (true);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_app_translation_key ON app_translation(translation_key_id);
CREATE INDEX IF NOT EXISTS idx_app_translation_language ON app_translation(language_code);

CREATE OR REPLACE TRIGGER trg_app_translation_updated_at
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'recipe_translation' AND policyname = 'public reads recipe_translation') THEN
    CREATE POLICY "public reads recipe_translation" ON recipe_translation FOR SELECT USING (true);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_recipe_translation_recipe ON recipe_translation(recipe_id);
CREATE INDEX IF NOT EXISTS idx_recipe_translation_language ON recipe_translation(language_code);

CREATE OR REPLACE TRIGGER trg_recipe_translation_updated_at
