-- Migration: add_generate_meal_plan_internal
-- Creates the internal batch meal-plan generation function, locks down execute
-- permissions to service_role/postgres only, and adds covering indexes for
-- per-user queries executed inside the function.
...
-- Security: restrict execute to service_role/postgres only.
-- Prevents any Supabase client from calling this via PostgREST RPC.
...
-- Indexes: covering indexes for per-user queries inside the function.
-- Idempotent with IF NOT EXISTS.
CREATE INDEX IF NOT EXISTS idx_meal_plan_user_start_active
  ON public.meal_plan (user_id, start_date, is_active);

CREATE INDEX IF NOT EXISTS idx_user_goal_user
  ON public.user_goal (user_id);

CREATE INDEX IF NOT EXISTS idx_nutrition_plan_user
  ON public.nutrition_plan (user_id);

CREATE INDEX IF NOT EXISTS idx_meal_distribution_plan
  ON public.meal_distribution (nutrition_plan_id);
