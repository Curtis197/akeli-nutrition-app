-- Fix: generate_meal_plan previously used UPDATE SET is_active=false to retire old plans,
-- leaving stale meal_plan rows that were all queried together causing duplicate entries
-- (e.g. 7 breakfast entries for a single day from 4 overlapping plans).
--
-- Correct behaviour:
--   • On regeneration, delete only plans whose date range overlaps the new range.
--   • Past non-overlapping plans are preserved for analytics.
--   • meal_plan_entry, cooking_session, and shopping_list all have ON DELETE CASCADE.
--
-- One-time cleanup: for each overlapping group, keep the most recent plan.

-- ── 1. One-time cleanup of overlapping duplicates ────────────────────────────
DELETE FROM meal_plan mp
WHERE EXISTS (
  SELECT 1 FROM meal_plan newer
  WHERE newer.user_id    =  mp.user_id
    AND newer.id        !=  mp.id
    AND newer.created_at >  mp.created_at
    AND newer.start_date <= mp.end_date
    AND newer.end_date   >= mp.start_date
);

-- ── 2. Replace generate_meal_plan ────────────────────────────────────────────
...
BEGIN
  ...
  -- Delete only plans that overlap the new date range.
  -- Past non-overlapping plans are preserved for analytics.
  -- meal_plan_entry, cooking_session, and shopping_list cascade automatically.
