-- supabase/migrations/20260608071931_cleanup_old_plan_sessions_on_replace.sql
--
-- Problem: generate_meal_plan_internal deactivated the old meal plan but left its
-- cooking_session and shopping_list rows orphaned. The next plan generation for
-- the same user would accumulate stale data.
--
-- Fix: before deactivating the old plan, delete its cooking_session and
-- shopping_list rows so only the newly created plan owns them.

DROP FUNCTION IF EXISTS public.generate_meal_plan_internal(uuid, integer, integer, date);

CREATE FUNCTION public.generate_meal_plan_internal(
  p_user_id       uuid,
...
BEGIN
  ...
  -- Delete cooking sessions and shopping list for the plan being replaced,
  -- so only the new active plan owns these records.
  DELETE FROM public.cooking_session
  WHERE meal_plan_id IN (
    SELECT id FROM public.meal_plan WHERE user_id = p_user_id AND is_active = true
  );
  ...
  -- 1. Batch cooking sessions (must run before shopping list)
  PERFORM public.create_batch_sessions_internal(v_plan_id, p_user_id, 7);
  -- 2. Shopping list (aggregates both batch and non-batch ingredients)
