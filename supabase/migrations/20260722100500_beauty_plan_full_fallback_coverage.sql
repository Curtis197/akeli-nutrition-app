...
-- 1x_week branches so they don't break if their frequency-specific pool
-- is empty.
-- File: supabase/migrations/20260722100500_beauty_plan_full_fallback_coverage.sql
--
-- Fixes: An uncommitted in-place edit on
-- 20260721000008_beauty_plan_fix_recommendations.sql attempted to add a
-- v_found fallback to the 2x_week branch but left out daily and 1x_week;
-- it also violated the append-only migration policy by modifying an
-- already-applied file. This additive migration re-declares
-- generate_beauty_plan with full fallback coverage across all frequency
-- branches (except 2x_month/1x_month, which don't need it because their
-- fallback is to insert nothing rather than breaking the plan). The
-- uncommitted diff on 20260721000008 is being reverted alongside this.
...
BEGIN
    ...
    WHILE v_curr_date <= v_end_date LOOP
        ...
        IF NOT v_found THEN
            FOR v_rec IN
                SELECT recipe_id, beauty_type, beauty_sub_type
                FROM recommend_recipes(p_user_id => p_user_id, p_limit => 2, p_mode => 'beauty'::TEXT)
            LOOP
                SELECT r.creator_id INTO v_row_creator_id FROM recipe r WHERE r.id = v_rec.recipe_id;
                IF v_fan_creator_id IS NULL OR v_other_count < v_max_other_slots OR v_row_creator_id = v_fan_creator_id THEN
                    INSERT INTO beauty_plan_slot (plan_id, day_number, week_number, day_of_week, routine_category, step_stage, recipe_id, frequency_tier)
