-- Migration: fix_generate_meal_plan_kcal_per_100g
-- Fixes a live production bug: generate_meal_plan() and
-- generate_meal_plan_from_saved() reference recipe_macro.calories_per_100g,
-- a column that does not exist (the actual generated column is
-- recipe_macro.kcal_per_100g). This has been broken since the per-100g
-- schema migrations (20260614200000 onward) were repeatedly superseded by
-- later rewrites (budget feature, variety days, etc.) that reintroduced the
-- wrong name. Confirmed reproducible: calling generate_meal_plan() for any
-- user without a recipe-preference vector (the common no-vector fallback
-- path) fails immediately with "column rm.calories_per_100g does not exist".
--
-- Also fixes a pre-existing IDOR: generate_meal_plan_from_saved() had no
-- auth.uid() check on p_user_id (unlike its sibling generate_meal_plan()),
-- so any authenticated caller could pass another user's uuid and generate/
-- modify that user's meal plan. Added the identical guard generate_meal_plan()
-- already uses.
--
-- Both functions replaced with their current bodies verbatim, with every
-- occurrence of calories_per_100g mechanically renamed to kcal_per_100g
-- (protein_per_100g/carbs_per_100g/fat_per_100g were already correct) plus
-- the one added auth guard in generate_meal_plan_from_saved. No other logic
-- changes.
