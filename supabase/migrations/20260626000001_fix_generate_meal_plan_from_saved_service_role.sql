-- Migration: 20260626000001_fix_generate_meal_plan_from_saved_service_role
-- Description: Remove auth.uid() guard from generate_meal_plan_from_saved so the batch
-- cron edge function (service_role context) can call it. auth.uid() is always NULL in
-- service_role — the guard blocked every user during batch processing.
-- Security is preserved via REVOKE from authenticated (batch cron is the sole caller,
-- already gated by INTERNAL_SECRET in the edge function).
