-- =============================================================================
-- Migration: 20260525000001_swap_meal_entry.sql
-- Description: Add goals to user_goal and create swap_meal_plan_entry RPC
-- =============================================================================

-- 1. Add nutrition goals to user_goal table
...
-- 2. Create swap_meal_plan_entry RPC
...
BEGIN
  ...
  SELECT mp.user_id, mp.id, mpe.date INTO v_user_id, v_plan_id, v_entry_date
  ...
  WHERE meal_plan_id = v_plan_id AND date = v_entry_date;
  ...
  IF v_calorie_goal IS NOT NULL AND v_calorie_goal > 0 AND v_recipe_calories IS NOT NULL AND v_recipe_calories > 0 THEN
    ...
    -- Ensure servings are at least 0.1
  ...
  -- 9. Regenerate the shopping list (updates existing items based on new components/servings)
