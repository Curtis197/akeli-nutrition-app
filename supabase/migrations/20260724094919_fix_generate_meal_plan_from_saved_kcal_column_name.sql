-- Migration: fix generate_meal_plan_from_saved referencing the
-- nonexistent column recipe_macro.kcal_per_100g (should be
-- calories_per_100g), causing "42703: column rm.kcal_per_100g does not
-- exist" on every call.
--
-- Root cause: 20260717061116_fix_generate_meal_plan_kcal_column_name.sql
-- redefined BOTH generate_meal_plan and generate_meal_plan_from_saved to
-- use calories_per_100g. Only the generate_meal_plan half of that fix
-- actually landed on production at the time; generate_meal_plan_from_saved
-- kept its old, broken kcal_per_100g body. This repo's migration
-- bookkeeping later marked 20260717061116 as fully applied based on
-- spot-checking only generate_meal_plan's source, so the gap in
-- generate_meal_plan_from_saved went unnoticed and 20260724072945
-- (a `supabase db pull` snapshot of actual production state) faithfully
-- captured the still-broken version.
--
-- Confirmed via supabase/tests/database/saved_recipe_eligibility.test.sql
-- ("small pool" scenarios), which call generate_meal_plan_from_saved and
-- died with 42703. No other logic differs from the currently live version
-- (verified against 20260717061116's body) — this is a column rename only.
