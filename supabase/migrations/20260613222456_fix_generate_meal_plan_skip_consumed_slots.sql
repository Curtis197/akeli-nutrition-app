-- Refine mid-week regen: also preserve consumed entries on the regen start day.
-- Previously: DELETE all entries >= p_start_date (could wipe lunch/dinner eaten
--             before a mid-day plan update).
-- Now:
--   DELETE only unconsumed entries >= p_start_date.
--   SKIP inserting a new entry for any (date, meal_type) that already has a
--   consumed entry (covers both past-day and mid-day consumed slots).
...
BEGIN
  ...
  IF v_existing_plan_id IS NOT NULL THEN
    -- Only delete unconsumed future entries so consumed meals (including
    -- meals eaten earlier on the regen day itself) are never lost.
  ...
  FOR v_day IN 0..(p_days - 1) LOOP
    ...
    FOREACH v_meal_type IN ARRAY v_meal_types LOOP
      -- Skip slots already covered by a consumed entry (handles mid-day regen).
      IF EXISTS (
