-- Fix: generate_meal_plan had an asymmetric feasibility filter.
--
-- Previous filter only excluded recipes too calorie-sparse to hit the target
-- within max_portion_g, but had no lower-bound check. A very calorie-dense
-- recipe (e.g. Akara at 557 kcal/100g) could be selected and then clamped UP
-- to min_portion_g, overshooting the calorie target significantly.
--
-- Fix: require the calorie-target-derived natural portion to fall within
-- [v_min_g, v_max_g]. If a recipe's natural portion < v_min_g it is excluded.
-- Applied to both the vector-path and no-vector-path SELECT branches.
    ...
    FOREACH v_meal_type IN ARRAY v_meal_types LOOP
      ...
      IF v_user_vector IS NOT NULL THEN
        ...
        WHERE r.is_published = true
          ...
          AND (v_target_meal_cal IS NULL OR (
                v_target_meal_cal / (rm.calories_per_100g / 100) BETWEEN v_min_g AND v_max_g
              ))
