-- Fix 1: generate_meal_plan passed hardcoded 7 to create_batch_sessions_internal.
--        Now passes p_days so shorter plans (e.g. today→Sunday = 3 days) get
--        the correct max_portions ceiling instead of a meaningless 7.
--
-- Fix 2: New function generate_initial_meal_plan for new-user onboarding.
--        Computes days from today to the coming Sunday (inclusive) so the
--        first plan always ends on Sunday, aligning with the Monday batch cycle.

-- ── Fix 1: rewrite generate_meal_plan passing p_days through ─────────────────
...
BEGIN
    ...
    FOREACH v_meal_type IN ARRAY v_meal_types LOOP
      ...
      IF v_user_vector IS NOT NULL THEN
        ...
        WHERE r.is_published = true
          ...
          AND (v_target_meal_cal IS NULL OR v_target_meal_cal / (rm.calories_per_100g / 100) <= v_max_g)
      ...
      IF v_fan_creator_id IS NOT NULL THEN
        IF v_recipe.creator_id = v_fan_creator_id THEN v_fan_count := v_fan_count + 1;
        ELSE v_other_count := v_other_count + 1;
        END IF;
      END IF;

      RETURN QUERY SELECT
        v_plan_id, v_entry_id, v_component_id, v_current_date, v_meal_type,
  ...
  -- p_days passed through: batch sessions respect actual plan length,
  -- not a hardcoded ceiling of 7.
  PERFORM public.create_batch_sessions_internal(v_plan_id, p_user_id, p_days);
  PERFORM public.generate_shopping_list_internal(v_plan_id, p_user_id);
END;
$$;

-- ── Fix 2: new-user onboarding function ──────────────────────────────────────
-- Generates today → coming Sunday (inclusive). Examples:
--   Sunday  (dow 0) → 1 day   (Monday batch covers next full week)
--   Monday  (dow 1) → 7 days  (identical to a batch week)
--   Wednesday(dow 3)→ 5 days
--   Saturday(dow 6) → 2 days
CREATE OR REPLACE FUNCTION public.generate_initial_meal_plan(
  p_user_id           uuid,
...
GRANT EXECUTE ON FUNCTION public.generate_initial_meal_plan(uuid, integer, integer)
  TO authenticated;
