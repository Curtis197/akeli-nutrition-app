-- 1. Add budget preferences to user_profile
...
-- 2. Raw scraped ingredient prices (One active price per ingredient per country)
CREATE TABLE IF NOT EXISTS public.ingredient_market_price (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ingredient_id UUID REFERENCES public.ingredient(id) ON DELETE CASCADE,
  country_code TEXT NOT NULL,         -- 'FR', 'GB', 'CA', 'US'
  currency TEXT NOT NULL DEFAULT 'EUR',
  price_per_100g NUMERIC(10,4) NOT NULL,
  source TEXT,                        -- e.g., 'carrefour', 'tesco', 'simulation_v1'
  scraped_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(ingredient_id, country_code) 
);

-- 3. Pre-calculated recipe costs (Keeps your RPCs lightning fast)
CREATE TABLE IF NOT EXISTS public.recipe_market_cost (
  recipe_id UUID REFERENCES public.recipe(id) ON DELETE CASCADE,
  country_code TEXT NOT NULL,
  cost_per_100g NUMERIC(10,4) NOT NULL,
  total_recipe_cost NUMERIC(10,2) NOT NULL,
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  PRIMARY KEY(recipe_id, country_code)
);

-- RLS Policies
ALTER TABLE public.ingredient_market_price ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.recipe_market_cost ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "public reads ingredient_market_price" ON public.ingredient_market_price;
CREATE POLICY "public reads ingredient_market_price" ON public.ingredient_market_price 
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "public reads recipe_market_cost" ON public.recipe_market_cost;
CREATE POLICY "public reads recipe_market_cost" ON public.recipe_market_cost 
  FOR SELECT USING (true);

-- 4. Function to recalculate recipe costs when ingredient prices change
