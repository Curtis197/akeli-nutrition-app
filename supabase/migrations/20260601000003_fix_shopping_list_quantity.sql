-- Fix RPC generate_shopping_list to prevent null constraint violations on quantity
-- when aggregating ingredients that have a NULL quantity in the recipe_ingredient table.
...
BEGIN
  ...
  WITH aggregated_ingredients AS (
    SELECT 
      ri.ingredient_id, 
      COALESCE(SUM(ri.quantity * mpe.servings), 0) AS quantity, 
      ri.unit
