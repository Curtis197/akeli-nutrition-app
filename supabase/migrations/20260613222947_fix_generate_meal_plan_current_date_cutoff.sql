-- Fix: always use CURRENT_DATE as the deletion cutoff, not p_start_date.
-- Past entries (< CURRENT_DATE) are never touched regardless of p_start_date.
-- Today and future entries are always regenerated.
-- The generation loop skips past dates so no duplicate entries are created.
...
BEGIN
  ...
  IF v_existing_plan_id IS NOT NULL THEN
    -- Delete today and future entries only. Anything before today is untouched.
