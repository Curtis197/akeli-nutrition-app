-- Fix: mid-week regen ignored preserved past days for variety, letting a single
-- recipe exceed the user's per-week max_recipe_repeat cap.
--
-- Root cause: on regen the function reuses the existing plan and skips past days
-- (scheduled_date < CURRENT_DATE), but v_used_recipe_ids started empty. The cap
-- filter `count(used) < p_max_recipe_repeat` therefore had no memory of recipes
-- already placed on the kept past days, so the regenerated slice (today→end)
-- re-stacked the top-scoring recipe up to the cap again. Across the week the
-- same recipe piled up far beyond the user's batch_cooking_max_portions setting.
--
-- Fix: after the plan id is resolved, seed v_used_recipe_ids from the base
-- components of the preserved past entries. The cap is then enforced over the
-- WHOLE plan, not just the regenerated slice. For a fresh plan (no past entries)
-- the seed is an empty array, so first-generation behaviour is unchanged.
...
BEGIN
  ...
  IF v_existing_plan_id IS NOT NULL THEN
    -- Alias 'e' disambiguates meal_plan_entry.meal_plan_id from the
  ...
  -- Seed the repeat counter with recipes already used in preserved past days
  -- (scheduled_date < CURRENT_DATE) so the per-week p_max_recipe_repeat cap is
  -- enforced across the WHOLE plan, not just the regenerated today→end slice.
  -- Only base components count, matching how the loop appends below. For a fresh
  -- plan there are no past entries, so this is an empty array (no behaviour change).
  ...
  FOR v_day IN 0..(p_days - 1) LOOP
    ...
    -- Skip past days — their entries already exist and were not deleted.
...
-- Restore the V3 hardening lost by later DROP+CREATE migrations: a SECURITY
-- DEFINER function in `public` is granted to PUBLIC by default, making it
-- callable by anon. The auth.uid() guard rejects anon calls, but revoke anyway
-- so only authenticated users can reach it (advisor lint 0028).
