-- Fix: generate_meal_plan and swap_meal_plan_entry were reading kcal_per_100g
-- (a GENERATED column = old per-serving calories / total_weight_g * 100),
-- which is 4-6x too low because recipe_macro.calories stores kcal PER SERVING,
-- not total recipe kcal.
--
-- The correct column is calories_per_100g, written by refresh_recipe_per_100g()
-- as: SUM(ingredient_weight * ingredient_kcal_per_100g / 100) / total_weight_g * 100
--
-- This migration:
--   1. Drops the misleading kcal_per_100g GENERATED column.
--   2. Rewrites generate_meal_plan to use calories_per_100g.
--   3. Rewrites swap_meal_plan_entry to use calories_per_100g.

-- 1. Drop the bad GENERATED column ─────────────────────────────────────────
ALTER TABLE public.recipe_macro
  DROP COLUMN IF EXISTS kcal_per_100g;

-- 2. Rewrite generate_meal_plan ─────────────────────────────────────────────
      ...
      IF v_user_vector IS NOT NULL THEN
        ...
        WHERE r.is_published = true
          ...
          AND (
            ...
            OR v_target_meal_cal / (rm.calories_per_100g / 100) <= 1500
          )
...
-- 3. Rewrite swap_meal_plan_entry ───────────────────────────────────────────
