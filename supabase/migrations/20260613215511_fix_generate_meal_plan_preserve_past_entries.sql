-- Fix: when regenerating mid-week, preserve past meal_plan_entry rows and their
-- is_consumed state. Only delete entries from p_start_date forward, reusing
-- the existing plan record so meal_consumption.meal_plan_entry_id links survive.
--
-- Previous behaviour: DELETE entire overlapping meal_plan → cascade-deleted all
-- entries, nulling meal_consumption.meal_plan_entry_id for already-logged meals.
--
-- New behaviour:
--   • If an overlapping plan exists → delete only entries ≥ p_start_date, reuse plan id.
--   • If no plan exists → create one as before.
--   • End_date is extended if the new range reaches further than the existing one.
...
BEGIN
  ...
  -- ── Plan reuse: preserve past entries ──────────────────────────────────────
  -- Find the most recent overlapping plan for this user.
  ...
  IF v_existing_plan_id IS NOT NULL THEN
    -- Delete only unconsumed entries from p_start_date forward.
    -- Already-consumed entries on the same day (e.g. lunch eaten before a
    -- mid-day regen) are preserved; the loop below skips those slots.
    DELETE FROM meal_plan_entry
    WHERE meal_plan_id    = v_existing_plan_id
      AND scheduled_date >= p_start_date
      AND is_consumed     = false;

    -- Extend end_date if the new range reaches further than the existing plan.
  ...
  FOR v_day IN 0..(p_days - 1) LOOP
    ...
    FOREACH v_meal_type IN ARRAY v_meal_types LOOP
      -- Skip slots already covered by a consumed entry (mid-day regen case).
      IF EXISTS (
