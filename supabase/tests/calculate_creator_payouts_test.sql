...
INSERT INTO auth.users (id, email, role, created_at, updated_at) VALUES
  ('a1000001-0000-0000-0000-000000000001', 'payout.owner@akeli.test', 'authenticated', now(), now()),
  ('a1000001-0000-0000-0000-000000000002', 'payout.creatorx@akeli.test', 'authenticated', now(), now()),
  ('a1000001-0000-0000-0000-000000000003', 'payout.creatory@akeli.test', 'authenticated', now(), now()),
  ('a1000001-0000-0000-0000-000000000004', 'payout.creatorz@akeli.test', 'authenticated', now(), now())
ON CONFLICT (id) DO NOTHING;

INSERT INTO user_profile (id, onboarding_done, is_creator, created_at, locale) VALUES
  ('a1000001-0000-0000-0000-000000000001', true, false, now(), 'fr'),
  ('a1000001-0000-0000-0000-000000000002', true, true, now(), 'fr'),
  ('a1000001-0000-0000-0000-000000000003', true, true, now(), 'fr'),
  ('a1000001-0000-0000-0000-000000000004', true, true, now(), 'fr')
ON CONFLICT (id) DO NOTHING;

INSERT INTO creator (id, user_id, display_name) VALUES
  ('a1000001-0000-0000-0000-000000000010', 'a1000001-0000-0000-0000-000000000002', 'Payout Creator X'),
  ('a1000001-0000-0000-0000-000000000011', 'a1000001-0000-0000-0000-000000000003', 'Payout Creator Y'),
  ('a1000001-0000-0000-0000-000000000012', 'a1000001-0000-0000-0000-000000000004', 'Payout Creator Z')
ON CONFLICT (id) DO NOTHING;

INSERT INTO recipe (id, creator_id, title, is_published, mode) VALUES
  ('a1000001-0000-0000-0000-000000000020', 'a1000001-0000-0000-0000-000000000010', 'Payout Test Recipe X', true, 'beauty'),
  ('a1000001-0000-0000-0000-000000000021', 'a1000001-0000-0000-0000-000000000011', 'Payout Test Recipe Y', true, 'beauty'),
  ('a1000001-0000-0000-0000-000000000022', 'a1000001-0000-0000-0000-000000000012', 'Payout Test Recipe Z', true, 'beauty')
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO beauty_plan_slot (plan_id, day_of_week, routine_category, step_stage, recipe_id, is_completed, completed_at, revenue_value) VALUES
  ('a1000001-0000-0000-0000-000000000030', 1, 'hair', 'daily_hydration', 'a1000001-0000-0000-0000-000000000020', true,  now(), 0.25),
  ('a1000001-0000-0000-0000-000000000030', 2, 'hair', 'daily_hydration', 'a1000001-0000-0000-0000-000000000020', true,  now(), 0.25),
  ('a1000001-0000-0000-0000-000000000030', 3, 'hair', 'daily_hydration', 'a1000001-0000-0000-0000-000000000020', true,  now(), 0.25),
  ('a1000001-0000-0000-0000-000000000030', 4, 'hair', 'daily_hydration', 'a1000001-0000-0000-0000-000000000020', false, NULL, 0.25);

-- Creator Y: 2 completed slots @ revenue_value 0.5 (sum 1.0).
INSERT INTO beauty_plan_slot (plan_id, day_of_week, routine_category, step_stage, recipe_id, is_completed, completed_at, revenue_value) VALUES
  ('a1000001-0000-0000-0000-000000000030', 5, 'skin', 'treatment', 'a1000001-0000-0000-0000-000000000021', true, now(), 0.5),
  ('a1000001-0000-0000-0000-000000000030', 6, 'skin', 'treatment', 'a1000001-0000-0000-0000-000000000021', true, now(), 0.5);

-- Creator Z: recipe exists in the plan, but its only slot is NOT completed —
-- Creator Z must get no payout row at all this month.
INSERT INTO beauty_plan_slot (plan_id, day_of_week, routine_category, step_stage, recipe_id, is_completed, completed_at, revenue_value) VALUES
  ('a1000001-0000-0000-0000-000000000030', 7, 'both', 'wash_day_mask', 'a1000001-0000-0000-0000-000000000022', false, NULL, 1.0);
...
SELECT is(
  (SELECT pool_earnings_cents FROM creator_monthly_payouts
   WHERE creator_id = 'a1000001-0000-0000-0000-000000000010'
...
SELECT is(
  (SELECT pool_earnings_cents FROM creator_monthly_payouts
   WHERE creator_id = 'a1000001-0000-0000-0000-000000000011'
     AND period_month = date_trunc('month', current_date)::date),
  100,
...
SELECT is(
  (SELECT count(*)::int FROM creator_monthly_payouts
   WHERE creator_id = 'a1000001-0000-0000-0000-000000000012'
     AND period_month = date_trunc('month', current_date)::date),
  0,
