...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (resendError) {
      console.error('[send-ingredient-approved-email] resend rejected the send:', resendError);
    ...
    console.log('[send-ingredient-approved-email] sent', { email, ingredientName, resendId: resendData?.id, at: new Date().toISOString() });
