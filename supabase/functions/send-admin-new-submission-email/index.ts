...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (resendError) {
      console.error('[send-admin-new-submission-email] resend rejected the send:', resendError);
    ...
    console.log('[send-admin-new-submission-email] sent', { name, resendId: resendData?.id, at: new Date().toISOString() });
