...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const recipeUrl = `https://akeli-admin-dashboard.vercel.app/recipes/${encodeURIComponent(recipeId)}`;
    const rows = [
    ...
    if (resendError) {
      console.error('[send-admin-new-recipe-email] resend rejected the send:', resendError);
    ...
    console.log('[send-admin-new-recipe-email] sent', { recipeId, title, resendId: resendData?.id, at: new Date().toISOString() });
