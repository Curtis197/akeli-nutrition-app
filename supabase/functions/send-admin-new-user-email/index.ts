...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (resendError) {
      console.error('[send-admin-new-user-email] resend rejected the send:', resendError);
    ...
    console.log('[send-admin-new-user-email] sent', { userId, resendId: resendData?.id, at: new Date().toISOString() });
