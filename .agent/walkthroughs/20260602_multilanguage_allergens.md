...
### 1. Multi-Language Allergens (English & French)
- **Database Schema**: Replaced the `label` column in the `allergen` table with explicit `label_fr` and `label_en` columns (mirroring the project's existing `food_region` translation pattern). Backfilled all 20 default allergens with their English and French equivalents.
- **RPC Update**: Updated the `search_allergens` RPC function to search across both `label_fr` and `label_en` columns simultaneously.
- **Flutter Model**: Updated `AllergenModel` in Dart to include `labelFr` and `labelEn` fields and added a `localizedLabel(locale)` getter.
- **UI Integration**: Updated `AllergenPickerWidget` to fetch the current app locale via `userProfileProvider` and seamlessly display the allergen label in the correct language (`a.localizedLabel(locale)`).
