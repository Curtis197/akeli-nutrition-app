...
class _CookingModePageState extends State<CookingModePage> {
  ...
  int _sectionStepCount(int headerIndex) {
    int count = 0;
    for (int i = headerIndex + 1; i < widget.recipe.steps.length; i++) {
      if (widget.recipe.steps[i].isSectionHeader) break;
      count++;
    }
    return count;
  }
              ...
              children: [
                ...
                Expanded(
                  child: PageView.builder(
                    ...
                    onPageChanged: (index) {
                      _logger.userAction('Step swiped',
                          screen: 'CookingModePage',
                          metadata: {'to': index + 1});
                      setState(() {
                        _currentStepIndex = index;
                    ...
                    itemBuilder: (_, index) {
                      ...
                      return Column(
                        children: [
                          ...
                          if (s.videoUrl != null || s.imageUrl != null)
                            Flexible(
                              flex: 3,
                              child: Padding(
                                padding: const EdgeInsets.symmetric(
                          ...
                          if (s.durationMin != null && index == _currentStepIndex)
                            _TimerWidget(
                              totalSeconds: s.durationMin! * 60,
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.restaurant_rounded,
                color: AkeliColors.primary, size: 36),
          ),
                    ...
                    itemBuilder: (_, index) {
                      ...
                      final sCount = () {
                        ...
                        for (int i = index + 1; i < recipe.steps.length; i++) {
                          if (recipe.steps[i].isSectionHeader) break;
                          count++;
                        }
                        return count;
                      }();
                      return _LandscapeCenter(
                        step: s,
                        stepIngredients: ingredients,
                        checked: checkedIngredients,
                        timerSeconds: index == currentStepIndex ? timerSeconds : 0,
                        timerRunning: index == currentStepIndex && timerRunning,
                        infoOpen: infoOpen,
                        onTimerToggle: onTimerToggle,
