...
SPECIALTY_SCRAPERS: dict = {}  # niche sources, populated for select countries only
GOOGLE_SCRAPERS:    dict = {}

def _build_scrapers():
    global OFF_SCRAPERS, RETAILER_SCRAPERS, SPECIALTY_SCRAPERS, GOOGLE_SCRAPERS
    ...
    SPECIALTY_SCRAPERS = {'US': AfroasiaUsScraper()}
...
def run_cascade(page, context, ingredient: dict, country_code: str):
    ...
    result, vr = _try(SPECIALTY_SCRAPERS.get(country_code), page)
    if result:
        return result, vr
